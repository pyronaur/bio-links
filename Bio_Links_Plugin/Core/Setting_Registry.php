...
class Setting_Registry {
	...
	public function is_valid( $setting ) {
		 ...
		 */
		if ( empty( $setting['id'] ) ) {
			trigger_error( 'Setting ID is required in `Bio_Links_Plugin\Settings\Setting_Registry`' );
	...
	public function add( $setting ) {
		 ...
		 */
		if ( $this->exists( $id ) ) {
			trigger_error( 'Setting `' . $id . '` already exists in `Bio_Links_Plugin\Settings\Setting_Registry`' );
